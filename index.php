...
if ($nv_Request->isset_request(NV_NAME_VARIABLE, 'get') or $nv_Request->isset_request(NV_NAME_VARIABLE, 'post')) {
    ...
    $alias_mod = $nv_Request->get_string(NV_NAME_VARIABLE, 'post,get');
...
if (preg_match($global_config['check_module'], $module_name)) {
	...
	$mod = '';
	// PAGE
	$count = count($array_url) - 1;
	$string = str_replace('.html','',end($array_url));
	if (substr($string, 0, 5) == 'page-') 
	{
			$page = intval(substr(end($array_url), 5));
			$alias_mod = str_replace('/','',$array_url[$count-1]);
	}
	else
	{
		$alias_mod = str_replace('/','',end($array_url));
		$alias_mod = str_replace('.html','',$alias_mod);
		$mod1 = $array_url[$count-1];
		if (isset($site_mods[$alias_mod]) and !empty($alias_mod))
		{
			//$module_name = '';
			$mod = $alias_mod;
			$alias_mod = $mod1;
