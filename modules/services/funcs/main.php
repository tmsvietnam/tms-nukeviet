...
if ($page_config['viewtype'] == 2) {
    ...
    // Xem theo bài viết
    $base_url_rewrite = nv_url_rewrite(NV_BASE_SITEURL . 'index.php?' . NV_LANG_VARIABLE . '=' . NV_LANG_DATA . '&' . NV_NAME_VARIABLE . '=' . $rowdetail['alias'], true);
    ...
    if ($related_articles) {
        ...
        while ($_other = $result->fetch()) {
            $_other['link'] = NV_BASE_SITEURL . 'index.php?' . NV_LANG_VARIABLE . '=' . NV_LANG_DATA . '&amp;' . NV_NAME_VARIABLE . '=' . $_other['alias'];
	...
	if(!empty($rowdetail['keywords']))
	{
		$array_tag_new = explode(',',$rowdetail['keywords']);
    ...
    $contents = nv_page_main($rowdetail, $array_keyword, $other_links, $content_comment);
} else {
    // Xem theo danh sách
