...
if ($nv_Request->get_int('save', 'post') == '1') {
	...
	$row['icon'] = $nv_Request->get_title('icon', 'post', '', 1);
   ...
   // $row['keywords'] = nv_strtolower($nv_Request->get_title('keywords', 'post', '', 0));
    ...
    elseif (empty($row['title'])) {
        ...
        if ($id and !$copy) {
            $_sql = 'UPDATE ' . NV_PREFIXLANG . '_' . $module_data . ' SET title = :title, alias = :alias, image = :image, imagealt = :imagealt, imageposition = :imageposition,titlesite = :titlesite,hometext = :hometext,icon = :icon,ratename = :ratename, ratenumber = :ratenumber,description = :description, bodytext = :bodytext, keywords = :keywords, socialbutton = :socialbutton, activecomm = :activecomm, layout_func = :layout_func, gid = :gid, edit_time = ' . NV_CURRENTTIME . ', hot_post = :hot_post WHERE id =' . $id;
            ...
            $_sql = 'INSERT INTO ' . NV_PREFIXLANG . '_' . $module_data . '
				(title, alias, image, imagealt, imageposition, titlesite,hometext,icon,ratename,ratenumber,description, bodytext, keywords, socialbutton, activecomm, layout_func, gid, weight,admin_id, add_time, edit_time, status,hot_post) VALUES
				(:title, :alias, :image, :imagealt, :imageposition, :titlesite,:hometext,:icon,:ratename,:ratenumber,:description, :bodytext, :keywords, :socialbutton, :activecomm, :layout_func, :gid, ' . $weight . ', ' . $admin_info['admin_id'] . ', ' . NV_CURRENTTIME . ', ' . NV_CURRENTTIME . ', 1,:hot_post)';
        ...
        try {
			...
			$sth->bindParam(':icon', $row['icon'], PDO::PARAM_STR);
...
$row['hometext'] = nv_htmlspecialchars(nv_br2nl($row['hometext']));
