...
if ($nv_Request->isset_request('getinfo', 'post')) {
    ...
    if ($id) {
        $sth = $db->prepare('SELECT title, phone,phone1,email,skype, image, start_time, end_time, status FROM ' . NV_PREFIXLANG . '_' . $module_data . '_rows WHERE id=:id');
...
if ($nv_Request->isset_request('submit', 'post')) {
    ...
    $data['phone1'] = nv_substr($nv_Request->get_title('phone1', 'post', ''), 0, 255);
    $data['email'] = nv_substr($nv_Request->get_title('email', 'post', ''), 0, 255);
    $data['skype'] = nv_substr($nv_Request->get_title('skype', 'post', ''), 0, 255);
    ...
    if (empty($error)) {
        if ($data['id']) {
            $sql = 'UPDATE ' . NV_PREFIXLANG . '_' . $module_data . '_rows SET
				title = :title,phone = :phone,phone1 = :phone1,email = :email,skype = :skype,  image = :image, start_time = :start_time, end_time = :end_time, status = :status
			WHERE id = ' . $data['id'];
        } else {
            $sql = 'INSERT INTO ' . NV_PREFIXLANG . '_' . $module_data . '_rows (bid, title,phone,phone1,email,skype,  image, start_time, end_time, status) VALUES (
				' . $bid . ', :title,:phone,:phone1,:email,:skype, :image, :start_time, :end_time, :status
			)';
        }
        ...
        try {
			...
			$sth->bindParam(':phone1', $data['phone1'], PDO::PARAM_STR);
			$sth->bindParam(':email', $data['email'], PDO::PARAM_STR);
			$sth->bindParam(':skype', $data['skype'], PDO::PARAM_STR);
