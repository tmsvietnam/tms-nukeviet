...
if (! nv_function_exists('nv_block_support')) {
    /**
     * nv_block_config_support()
     *
     * @param mixed $module
     ...
     * @return
     */
    function nv_block_config_support($module, $data_block, $lang_block)
    {
    ...
    /**
     * nv_block_config_support_submit()
     *
     * @param mixed $module
     * @param mixed $lang_block
     * @return
     */
    function nv_block_config_support_submit($module, $lang_block)
    {
        global $nv_Request;
        $return = array();
        $return['error'] = array();
     ...
     * @return
     */
    function nv_block_support($block_config)
    {
        ...
        $sql = 'SELECT id, title,phone,phone1,email,skype, image FROM ' . NV_PREFIXLANG . '_' . $site_mods[$module]['module_data'] . '_rows WHERE status = 1 AND bid = ' . $block_config['blockid'];
        $list = $nv_Cache->db($sql, 'id', $module);

        if (! empty($list)) {
            if (file_exists(NV_ROOTDIR . '/themes/' . $global_config['module_theme'] . '/modules/' . $site_mods[$module]['module_file'] . '/block.support.tpl')) {
                $block_theme = $global_config['module_theme'];
            } elseif (file_exists(NV_ROOTDIR . '/themes/' . $global_config['site_theme'] . '/modules/' . $site_mods[$module]['module_file'] . '/block.support.tpl')) {
                $block_theme = $global_config['site_theme'];
            } else {
                $block_theme = 'default';
            }

            $xtpl = new XTemplate('block.support.tpl', NV_ROOTDIR . '/themes/' . $block_theme . '/modules/' . $site_mods[$module]['module_file']);
$xtpl->assign('BLOCK_THEME', $block_theme);
            ...
            foreach ($list as $row) {
                ...
                if (! empty($row['image'])) { $xtpl->parse('main.loop.image');}
                 if (! empty($row['email'])) { $xtpl->parse('main.loop.email');}
                 if (! empty($row['skype'])) { $xtpl->parse('main.loop.skype');}
...
if (defined('NV_SYSTEM')) {
    $content = nv_block_support($block_config);
}
