...
$_query = $db->query('SELECT t1.title, t1.alias FROM ' . NV_PREFIXLANG . '_alias t1 INNER JOIN ' . NV_MOD_TABLE . '_tags_id t2 ON t1.id = t2.did WHERE t1.status= 1 AND t2.id ='.$row['id']);
while ($_row = $_query->fetch()) {
    $array_keyword[] = $_row;
    $meta_property['article:tag'][] = $_row['title'];
}
