...
if (! empty($page_title) and $page_title == strip_punctuation($page_title)) {
    $stmt = $db_slave->prepare('SELECT tid, title, titlesite, image, description,bodytext,ratename,ratenumber, keywords FROM ' . NV_PREFIXLANG . '_' . $module_data . '_tags WHERE alias= :alias');
    $stmt->bindParam(':alias', $alias, PDO::PARAM_STR);
    $stmt->execute();
    list($tid, $title,$titlesite,$image_tag, $description,$bodytext,$ratename,$ratenumber, $key_words) = $stmt->fetch(3);
    ...
    if ($tid > 0) {
        ...
        while ($item = $result->fetch()) {
            ...
            $item['link'] = NV_BASE_SITEURL . 'index.php?' . NV_LANG_VARIABLE . '=' . NV_LANG_DATA . '&amp;' . NV_NAME_VARIABLE . '=' . $item['alias'];
            $item_array[] = $item;
        }
        $result->closeCursor();
        unset($query, $row);
        ...
        $contents = topic_theme($item_array, $item_array_other, $generate_page, $page_title, $title,$bodytext,$ratename,$ratenumber,$description, $image_tag);
