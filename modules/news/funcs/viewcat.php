...
$base_url_rewrite = NV_BASE_SITEURL . 'index.php?' . NV_LANG_VARIABLE . '=' . NV_LANG_DATA . '&' . NV_NAME_VARIABLE . '=' . $global_array_cat[$catid]['alias'];
if ($page > 1) {
    ...
    if ($viewcat == 'viewcat_page_new' or $viewcat == 'viewcat_page_old' or $set_view_page) {
        ...
        if ($global_array_cat[$catid]['featured'] != 0) {
            ...
            if ($item = $result->fetch()) {
                ...
                $item['link'] = $global_array_cat[$catid]['rewrite_link'] . '/' . $item['alias'] . $global_config['rewrite_exturl'];
                $array_catpage[] = $item;
                $featured = $item['id'];
            }
        }
        ...
        while ($item = $result->fetch()) {
            ...
            $item['newday'] = $global_array_cat[$catid]['newday'];
            $item['link'] = $item['alias'] . $global_config['rewrite_exturl'];
            $array_catpage[] = $item;
            ...
            foreach ($array_catid as $catid_i) {
                ...
                if ($global_array_cat[$catid_i]['featured'] != 0) {
                    ...
                    if ($item = $result->fetch()) {
                        ...
                        $item['link'] = $global_array_cat[$catid_i]['link'] . '/' . $item['alias'] . $global_config['rewrite_exturl'];
