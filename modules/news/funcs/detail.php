...
if (nv_user_in_groups($global_array_cat[$catid]['groups_view'])) {
    ...
    if ($news_contents['id'] > 0) {
		...
		$base_url_rewrite = nv_url_rewrite(NV_BASE_SITEURL . 'index.php?' . NV_LANG_VARIABLE . '=' . NV_LANG_DATA . '&' . NV_NAME_VARIABLE . '=' . $news_contents['alias'], true);
    ...
    $array_keyword = array();
    $key_words = array();
     $_query = $db_slave->query('SELECT t1.title, t1.alias FROM ' . NV_PREFIXLANG . '_alias t1 INNER JOIN ' . NV_PREFIXLANG . '_' . $module_data . '_tags_id t2 ON t1.id = t2.tid WHERE t1.status= 1 AND t2.id ='.$id);
    while ($row = $_query->fetch()) {
		
        $array_keyword[] = $row;
        $key_words[] = $row['title'];
        $meta_property['article:tag'][] = $row['title'];
    }
	//print_r('SELECT a1.keyword, a2.alias FROM ' . NV_PREFIXLANG . '_' . $module_data . '_tags_id a1 INNER JOIN ' . NV_PREFIXLANG . '_' . $module_data . '_tags a2 ON a1.tid=a2.tid WHERE a1.id=' . $news_contents['id']);die;
    // comment
