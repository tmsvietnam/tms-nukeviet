    ...
    function nv_news_block_news($block_config, $mod_data)
    {
        ...
        if (($cache = $nv_Cache->getItem($module, $cache_file)) != false) {
            ...
            while (list ($id, $catid, $publtime, $exptime, $title, $alias, $homeimgthumb, $homeimgfile, $hometext, $external_link) = $result->fetch(3)) {
                $link = NV_BASE_SITEURL . 'index.php?' . NV_LANG_VARIABLE . '=' . NV_LANG_DATA . '&amp;' . NV_NAME_VARIABLE . '=' . $alias;
                if ($homeimgthumb == 1) {
                    //image thumb
        ...
        if ($module == $module_name) {
            ...
            if (!empty($list)) {
                foreach ($list as $l) {
                    ...
                    $module_array_cat[$l['catid']]['link'] = NV_BASE_SITEURL . 'index.php?' . NV_LANG_VARIABLE . '=' . NV_LANG_DATA . '&amp;' . NV_NAME_VARIABLE . '=' . $l['alias'];
                }
            }
        }
