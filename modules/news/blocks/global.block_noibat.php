...
if (! nv_function_exists('nv_block_noibat')) {
    function nv_block_config_noibat($module, $data_block, $lang_block)
    {
    ...
    function nv_block_config_noibat_submit($module, $lang_block)
    {
        global $nv_Request;
        $return = array();
        $return['error'] = array();
    ...
    function nv_block_noibat($block_config)
    {
        ...
        if (! empty($list)) {
            if (file_exists(NV_ROOTDIR . '/themes/' . $global_config['module_theme'] . '/modules/news/block_noibat.tpl')) {
                $block_theme = $global_config['module_theme'];
            } else {
                $block_theme = 'default';
            }
            $xtpl = new XTemplate('block_noibat.tpl', NV_ROOTDIR . '/themes/' . $block_theme . '/modules/news');
...
$a=1;
            foreach ($list as $l) {
				...
				if( $l['homeimgfile'] != '' and ( $imginfo = nv_is_image( NV_UPLOADS_REAL_DIR . '/' . $site_mods[$module]['module_upload'] . '/' . $l['homeimgfile'] ) ) != array() )
				{
					$image = NV_UPLOADS_REAL_DIR . '/' . $site_mods[$module]['module_upload'] . '/' . $l['homeimgfile'];

					if( $imginfo['width'] <= $width and $imginfo['height'] <= $height )
					{
						$l['homeimgfile'] = NV_BASE_SITEURL . NV_UPLOADS_DIR . '/' . $site_mods[$module]['module_upload'] . '/' . $l['homeimgfile'];
					}
					else
					{
						...
						if( file_exists( NV_ROOTDIR . '/' . NV_TEMP_DIR . '/' . $basename ) )
						{
							...
							$l['homeimgfile'] = NV_BASE_SITEURL . NV_TEMP_DIR . '/' . $basename;
						}
						else
						{
				...
				{
					$l['homeimgfile'] = $l['homeimgfile'];
				}

				else
				{
					$l['homeimgfile'] = NV_BASE_SITEURL . 'themes/' . $global_config['site_theme'] . '/images/no_image.gif';
                ...
                $xtpl->assign('ROW', $l);
               if (! empty($l['homeimgfile'])) {
					   $xtpl->parse('main.loop.img');
                    $xtpl->parse('main.top.img');
                }
				
				if($a==1){ $xtpl->parse('main.top');}
                else {$xtpl->parse('main.loop');}
...
if (defined('NV_SYSTEM')) {
    ...
    if (isset($site_mods[$module])) {
        ...
        $content = nv_block_noibat($block_config);
    }
}
