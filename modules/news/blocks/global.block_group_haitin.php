...
if (! nv_function_exists('nv_block_duan')) {
    function nv_block_config_duan($module, $data_block, $lang_block)
    {
    ...
    function nv_block_config_duan_submit($module, $lang_block)
    {
        global $nv_Request;
        $return = array();
        $return['error'] = array();
    ...
    function nv_block_duan($block_config)
    {
        ...
        $db->sqlreset()
            ->select('t1.id, t1.catid, t1.title, t1.alias, t1.homeimgfile, t1.homeimgthumb,t1.hometext,t1.publtime,t1.hitstotal,t1.external_link')
        ...
        if (! empty($list)) {
            if (file_exists(NV_ROOTDIR . '/themes/' . $global_config['module_theme'] . '/modules/news/block_haitin.tpl')) {
                $block_theme = $global_config['module_theme'];
            } else {
                $block_theme = 'default';
            }
            $xtpl = new XTemplate('block_haitin.tpl', NV_ROOTDIR . '/themes/' . $block_theme . '/modules/news');
            ...
            foreach ($list as $l) {
				...
				$l['title_clean'] = nv_clean60(strip_tags($l['title']),50);
				$l['hometext'] = nv_clean60(strip_tags($l['hometext']),150);
                ...
                if ($l['external_link']) {
				 ...
				 if ($a == 1) {
                  ...
                  $xtpl->parse('main.home1');
                }
				
				if ($a == 2) {
                   $xtpl->parse('main.home2');
                }

					else { if (! empty($l['thumb'])) {
...
if (defined('NV_SYSTEM')) {
    ...
    if (isset($site_mods[$module])) {
        ...
        $content = nv_block_duan($block_config);
    }
}
