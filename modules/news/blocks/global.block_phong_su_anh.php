...
if (! nv_function_exists('nv_block_phong_su_anh')) {
    function nv_block_config_phong_su_anh($module, $data_block, $lang_block)
    {
    ...
    function nv_block_config_phong_su_anh_submit($module, $lang_block)
    {
        global $nv_Request;
        $return = array();
        $return['error'] = array();
    ...
    function nv_block_phong_su_anh($block_config)
    {
        ...
        if (! empty($list)) {
            if (file_exists(NV_ROOTDIR . '/themes/' . $global_config['module_theme'] . '/modules/news/block_phong_su_anh.tpl')) {
                $block_theme = $global_config['module_theme'];
            } else {
                $block_theme = 'default';
            }
            $xtpl = new XTemplate('block_phong_su_anh.tpl', NV_ROOTDIR . '/themes/' . $block_theme . '/modules/news');
            ...
            foreach ($list as $l) {
                ...
                $l['hometext_clean'] = nv_clean60($l['hometext_clean'], 100);
				$l['ngay'] = nv_date('d  ', $l['publtime']);
				$l['nam'] = nv_date('m/Y ', $l['publtime']);
...
if (defined('NV_SYSTEM')) {
    ...
    if (isset($site_mods[$module])) {
        ...
        $content = nv_block_phong_su_anh($block_config);
    }
}
