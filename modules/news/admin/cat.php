...
if (!empty($savecat)) {
	...
	$check_return = $check_alias->check_catid_alias($catid, $alias);
    ...
    if ($catid == 0 and $title != '' and $error =='') {
        ...
        if ($newcatid > 0) {
			...
			// Thêm id_alias
			$check_alias->add_alias_catalogy_news($newcatid, $alias, $module_name, 'viewcat');
    ...
    } elseif ($catid > 0 and $title != '' and $error =='') {
        ...
        if ($stmt->rowCount()) {
			...
			// Thêm id_alias
			$check_alias->add_alias_catalogy_news($catid, $alias, $module_name, 'viewcat');
