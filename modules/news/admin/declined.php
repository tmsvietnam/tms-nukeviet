    ...
    while (list($id, $listcatid, $publtime, $exptime, $status) = $result->fetch(3)) {
        if (($exptime == 0 or $exptime > NV_CURRENTTIME) and $status != 4 and $status <= $global_code_defined['row_locked_status']) {
            ...
            if ($check_permission > 0) {
                $db->query('UPDATE ' . NV_PREFIXLANG . '_' . $module_data . '_rows SET status = 6 WHERE id =' . $id);
                foreach ($arr_catid as $catid_i) {
                    $db->query('UPDATE ' . NV_PREFIXLANG . '_' . $module_data . '_' . $catid_i . ' SET status = 6 WHERE id =' . $id);
                }
                $exp_array[] = $id;
            }
        }
