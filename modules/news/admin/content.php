...
if ($is_submit_form) {
    ...
    if (empty($error)) {
        ...
        if ($rowcontent['id'] == 0 or $copy) {
			...
			// Thêm id_alias
			$check_alias->add_alias_news($rowcontent['catid'],$rowcontent['id'], $rowcontent['alias'], $module_name, 'detail');
        ...
        if (empty($error)) {
			...
			if ($rowcontent['tags'] != $rowcontent['tags_old'] or $copy) {
				...
				$db->query('DELETE FROM ' . NV_PREFIXLANG . '_' . $module_data . '_tags_id WHERE id = ' . $rowcontent['id']);
				
				// THÊM LẠI VÀO 
				if(!empty($rowcontent['tags']))
				{
					$array_tag = explode(',',$rowcontent['tags']);
					foreach($array_tag as $tag_new)
					{
						$db->query('INSERT INTO ' . NV_PREFIXLANG . '_' . $module_data . '_tags_id (id, tid, keyword) VALUES (' . $rowcontent['id'] . ', ' . intval($tag_new) . ', ' . intval($tag_new) . ')');
					}
				}
			}
...
//print_r($list_tag_news);die;
// DANH SÁCH ALIAS CỦA ID NÀY
if($rowcontent['id'] > 0)
{
	$list_tag_id = $db->query('SELECT tid FROM ' . NV_PREFIXLANG . '_' . $module_data . '_tags_id WHERE id ='. $rowcontent['id'])->fetchAll();
}
//print_r($list_tag_id);die;
$array_tag_new = array();
