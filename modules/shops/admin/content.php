...
if ($nv_Request->get_int('save', 'post') == 1) {
	...
	// CẤU HÌNH GIÁ
	$title_setting = $nv_Request->get_array('title_setting', 'post', array());
	$price_setting = $nv_Request->get_array('price_setting', 'post', array());
	
	$array_setting_price = array();
	$dem = count($title_setting);
	for($i = 0; $i < $dem ; $i++)
	{
		$array_setting_price[$i]['title'] = $title_setting[$i];
		$array_setting_price[$i]['price'] = $price_setting[$i];
	}
	
	$rowcontent['setting_price'] = json_encode($array_setting_price);
	...
	$check_return = $check_alias->check_id_alias($rowcontent['id'], $rowcontent['alias']);
    ...
    if (empty($error)) {
        ...
        if ($rowcontent['id'] == 0 or $is_copy) {
            ...
            $data_insert['setting_price'] = $rowcontent['setting_price'];
            ...
            if ($rowcontent['id'] > 0) {
				...
				// Thêm id_alias
				$check_alias->add_alias_news($rowcontent['listcatid'],$rowcontent['id'], $rowcontent['alias'], $module_name, 'detail');
            ...
            $stmt->bindParam(':setting_price', $rowcontent['setting_price'], PDO::PARAM_STR);
        ...
        if ($error == '') {
			...
			if ($rowcontent['keywords'] != $rowcontent['keywords_old'] || ($is_copy == 1 && $rowcontent['keywords'] !='')) {
				...
				$db->query('DELETE FROM ' . $db_config['prefix'] . '_' . $module_data . '_tags_id_' . NV_LANG_DATA . ' WHERE id = ' . $rowcontent['id']);
				
				// THÊM LẠI VÀO 
				if(!empty($rowcontent['keywords']))
				{
					$array_tag = explode(',',$rowcontent['keywords']);
					foreach($array_tag as $tag_new)
					{
						$db->query('INSERT INTO ' . $db_config['prefix'] . '_' . $module_data . '_tags_id_' . NV_LANG_DATA . ' (id, tid, keyword) VALUES (' . $rowcontent['id'] . ', ' . intval($tag_new) . ', ' . intval($tag_new) . ')');
					}
				}
			}
...
if (!empty($rowcontent['setting_price']))
{
	$array_price = json_decode($rowcontent['setting_price'],true);
	
	foreach($array_price as $price_s)
	{
		$xtpl->assign('price_s', $price_s);
		$xtpl->parse('main.price_s');
	}
...
if($rowcontent['id'] > 0)
{
	$list_tag_id = $db->query('SELECT tid FROM ' . $db_config['prefix'] . '_' . $module_data . '_tags_id_' . NV_LANG_DATA . ' WHERE id ='. $rowcontent['id'])->fetchAll();
}
