...
if (!empty($savecat)) {
	...
	$check_return = $check_alias->check_catid_alias($data['catid'], $data['alias']);
    ...
    if ($data['catid'] == 0 and $data['title'] != '' and $error == '') {
        ...
        if ($newcatid > 0) {
			...
			// Thêm id_alias
			$check_alias->add_alias_catalogy_news($newcatid, $data['alias'], $module_name, 'viewcat');
        ...
        try {
            ...
            if ($error == '') {
                ...
                if ($stmt->execute()) {
					...
					// Thêm id_alias
					$check_alias->add_alias_catalogy_news($data['catid'], $data['alias'], $module_name, 'viewcat');
...
if ($error != '') {
    $xtpl->assign('CAT_LIST', shops_show_cat_list($data['parentid_old']));
