    ...
    if ($result->rowCount()) {
		...
		$array_price = json_decode($setting_price,true);
     	 ...
     	 $data_pro[] = array(
            ...
            'title' => $title,
            'alias' => $alias,
            'product_price' => $listprice[$i] + $array_price['price'],
            'product_price_total' => ($listprice[$i] + $array_price['price']) * $listnum[$i],
	...
	if(!empty($pdata['setting_price']))
		{
				...
				$xtpl->parse('main.loop.setting_price');
...
$data_content['coupons'] = $db->query('SELECT amount FROM ' . $db_config['prefix'] . '_' . $module_data . '_coupons_history WHERE order_id ='.$data_content['order_id'])->fetchColumn();
