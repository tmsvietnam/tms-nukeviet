...
if ($nv_Request->isset_request('submit', 'post')) {
    ...
    $row['free_ship'] = $nv_Request->get_title('free_ship', 'post', '');
    ...
    if (empty($error)) {
        try {
            if (empty($row['id'])) {
                $sql = 'INSERT INTO ' . $db_config['prefix'] . '_' . $module_data . '_carrier_config_items (cid, title, description, weight, free_ship, add_time) VALUES (:cid, :title, :description, :weight, :free_ship, ' . NV_CURRENTTIME . ' )';
                ...
                $data_insert['free_ship'] = $row['free_ship'];
                $insert_id = $db->insert_id($sql, 'id', $data_insert);
            } else {
                $stmt = $db->prepare('UPDATE ' . $db_config['prefix'] . '_' . $module_data . '_carrier_config_items SET cid = :cid, title = :title, description = :description, free_ship =:free_ship WHERE id=' . $row['id']);
                $stmt->bindParam(':title', $row['title'], PDO::PARAM_STR);
                $stmt->bindParam(':cid', $row['cid'], PDO::PARAM_STR);
                $stmt->bindParam(':description', $row['description'], PDO::PARAM_STR);
                $stmt->bindParam(':free_ship', $row['free_ship'], PDO::PARAM_STR);
