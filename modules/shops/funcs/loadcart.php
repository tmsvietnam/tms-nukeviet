...
if ($nv_Request->isset_request('get_shipping_price', 'get')) {
	...
	$total = $nv_Request->get_title('total', 'get', 0);
	$total = str_replace(".","",$total);

    $ship_price = nv_shipping_price($total, $weight, $weight_unit, $location_id, $shops_id, $carrier_id);
    if (!empty($ship_price)) {
        $total += $ship_price;
    }
}
