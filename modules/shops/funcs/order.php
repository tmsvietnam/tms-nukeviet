...
if ($post_order == 1) {
    ...
    $i = 0;
    $listid = $list_order = $listnum = $listprice = $listgroup = $listid_old = $listnum_old = array();
    ...
    foreach ($_SESSION[$module_data . '_cart'] as $pro_id => $info) {
        ...
        if ($_SESSION[$module_data . '_cart'][$proid]['order'] == 1) {
			...
			$array_price = json_decode($info['setting_price'],true);
			
			if($array_price['price'] > 0)
			{
			$price['sale'] = $price['sale'] + ($array_price['price'] * $info['num']);
			}
    ...
    if ($data_order['order_shipping']) {
        ...
        $price_ship = nv_shipping_price($total, $total_weight, $pro_config['weight_unit'], $data_order['shipping']['ship_location_id'], $data_order['shipping']['ship_shops_id'], $data_order['shipping']['ship_carrier_id']);
   ...
   // $total += $total_weight_price;
    ...
    $data_order['order_total'] = $total + $total_weight_price;
    ...
    if (empty($error) and $i > 0) {
        if (!empty($order_info)) {
            ...
            if ($sth->execute()) {
				...
				// CẬP NHẬT LẠI MÃ KHUYẾN MÃI
				if(!empty($array_counpons['code']))
				{//print_r($total_old);die;
					$amount = $total_old - $total;
					
					$db->query('UPDATE ' . $db_config['prefix'] . '_' . $module_data . '_coupons_history SET cid = '. $counpons['id'].', amount ='. $amount .' WHERE order_id ='. $order_info['order_id']);
				}
            }
        } else {
        ...
        if ($order_id > 0) {
            ...
            foreach ($_SESSION[$module_data . '_cart'] as $pro_id => $info) {
                ...
                if ($_SESSION[$module_data . '_cart'][$proid]['order'] == 1 and $i > 0) {
                    ...
                    $info['price'] = $price['sale'];
					//$info['id_setting_price'] = 1;
                    $sql = 'INSERT INTO ' . $db_config['prefix'] . '_' . $module_data . '_orders_id( order_id,listgroupid,id_setting_price,setting_price, proid, num, price, discount_id ) VALUES ( :order_id,:listgroupid,:id_setting_price,:setting_price, :proid, :num, :price, :discount_id )';
                    $data_insert = array();
                    ...
                    $data_insert['id_setting_price'] = $info['id_setting_price'];
                    $data_insert['setting_price'] = $info['setting_price'];
            ...
            if (!empty($list_order)) {
				...
				foreach($list_order as $id_order)
				{
				
					$sql = 'SELECT t3.num, t3.setting_price, t3.listgroupid, t1.id, t1.listcatid, t1.publtime, t1.' . NV_LANG_DATA . '_title, t1.' . NV_LANG_DATA . '_alias, t1.' . NV_LANG_DATA . '_hometext, t2.' . NV_LANG_DATA . '_title, t1.money_unit FROM ' . $db_config['prefix'] . '_' . $module_data . '_rows AS t1 LEFT JOIN ' . $db_config['prefix'] . '_' . $module_data . '_units AS t2 ON t1.product_unit = t2.id LEFT JOIN ' . $db_config['prefix'] . '_' . $module_data . '_orders_id AS t3 ON t3.proid = t1.id WHERE t3.id ='. $id_order .' AND t1.status =1';
					$result = $db->query($sql);

					while (list($num, $setting_price, $listgroupid, $id, $listcatid, $publtime, $title, $alias, $hometext, $unit, $money_unit) = $result->fetch(3)) {
			...
			// LẤY THÔNG TIN MÃ KHUYỄN MÃI
			$data_order['coupons'] = $db->query('SELECT amount FROM ' . $db_config['prefix'] . '_' . $module_data . '_coupons_history WHERE order_id ='.$data_order['id'])->fetchColumn();
			
			// LẤY THÔNG TIN VẬN CHUYỂN
			$result = $db->query('SELECT * FROM ' . $db_config['prefix'] . '_' . $module_data . '_orders_shipping WHERE order_id = ' . $data_order['id']);
			$data_shipping = $result->fetch();
			
            $email_contents_table = call_user_func('email_new_order', $content, $data_order, $data_pro, $data_shipping, true);
            $replace_data = array(
...
if ($action == 0) {
    ...
    foreach ($_SESSION[$module_data . '_cart'] as $pro_id => $pro_info) {
        ...
        if (true) {
            ...
            t1.money_unit, t1.discount_id, t1.product_weight, t1.weight_unit, t2." . NV_LANG_DATA . "_title FROM " . $db_config['prefix'] . "_" . $module_data . "_rows AS t1,
            ...
            $sql = "SELECT t1.id, t1.listcatid, t1.publtime, t1." . NV_LANG_DATA . "_title, t1." . NV_LANG_DATA . "_alias,
            t1." . NV_LANG_DATA . "_hometext, t1.homeimgalt, t1.homeimgfile, t1.homeimgthumb, t1.product_price, t1.product_number,
            t1.money_unit, t1.discount_id, t1.product_weight, t1.weight_unit, t2." . NV_LANG_DATA . "_title FROM " . $db_config['prefix'] . "_" . $module_data . "_rows AS t1, " . $db_config['prefix'] . "_" . $module_data . "_units AS t2,
        ...
        while (list(
            $id, $listcatid, $publtime, $title, $alias, $hometext, $homeimgalt, $homeimgfile, $homeimgthumb, $product_price, $product_number, $money_unit, $discount_id, $product_weight, $weight_unit, $unit,
        ) = $result->fetch(3)) {
            if ($homeimgthumb == 1) {
                //image thumb
            ...
            $num = $_SESSION[$module_data . '_cart'][$id . '_' . $array[1]. '_' . $array[2]]['num'];
