...
if (nv_user_in_groups($global_array_shops_cat[$catid]['groups_view'])) {
    ...
    $base_url_rewrite = nv_url_rewrite(NV_BASE_SITEURL . 'index.php?' . NV_LANG_VARIABLE . '=' . NV_LANG_DATA . '&amp;' . NV_NAME_VARIABLE . '=' . $data_content[NV_LANG_DATA . '_alias'], true);
    ...
    $array_keyword = array();
    $key_words = array();
	$_query = $db->query('SELECT t1.title, t1.alias FROM ' . NV_PREFIXLANG . '_alias t1 INNER JOIN ' . $db_config['prefix'] . '_' . $module_data . '_tags_id_' . NV_LANG_DATA . ' t2 ON t1.id = t2.tid WHERE t1.status= 1 AND t2.id ='.$data_content['id']);
