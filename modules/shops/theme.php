...
function nv_template_detail($data_content, $data_unit, $data_others, $array_other_view, $content_comment, $compare_id, $popup, $idtemplate, $array_keyword)
{
    ...
    if (!empty($data_content)) {
		...
		if (!empty($data_content['setting_price']))
		{
			$array_price = json_decode($data_content['setting_price'],true);
			
			$i = 0;
			foreach($array_price as $price_s)
			{
				$xtpl->assign('i', $i);
				$i++;
				$xtpl->assign('price_s', $price_s);
				if($price_s['price'] > 0)
				$xtpl->assign('dau', '+');
				else $xtpl->assign('dau', '-');
				$xtpl->parse('main.setting_price.price_s');
			
			}
			
			$xtpl->parse('main.setting_price');
		}
        ...
        if (!$popup) {
            ...
            if (!empty($array_keyword)) {
                ...
                foreach ($array_keyword as $i => $value) {
                    $xtpl->assign('KEYWORD', $value['title']);
...
function cart_product($data_content, $coupons_code, $order_info, $array_error_number)
{
    ...
    if (!empty($data_content)) {
        ...
        foreach ($data_content as $data_row) {
			...
			if(!empty($data_row['setting_price']))
			{
				$array_price = json_decode($data_row['setting_price'],true);
				
				
				$price['sale_format'] = number_format($price['sale'] + ($array_price['price'] * $data_row['num']),0,",",".");
				
				if($array_price['price'] > 0)
				$xtpl->assign('dau', '+');
				else $xtpl->assign('dau', '-');
				$array_price['price'] = number_format($array_price['price'],0,",",".");
				$xtpl->assign('setting_price', $array_price);
                $xtpl->parse('main.rows.price2.setting_price');
                $xtpl->parse('main.rows.price5.setting_price');
            ...
            $xtpl->assign('id_setting_price', $data_row['id_setting_price']);
    ...
    if (!empty($data_content)) {
        foreach ($data_content as $data_row) {
			...
			if(!empty($data_row['setting_price']))
			{
				...
				$price['sale'] = $price['sale'] + ($array_price['price'] * $data_row['num']);
    ...
    if ($module_config[$module_name]['use_shipping']) {
        ...
        if (!empty($shipping_data['list_carrier'])) {
            ...
            foreach ($shipping_data['list_carrier'] as $rows_i) {
                $rows_i['checked'] = ($i == 0) ? ' checked="checked"' : '';
...
function payment($data_content, $data_pro, $data_shipping, $payment_supported, $intro_pay, $point)
{
    ...
    foreach ($data_pro as $pdata) {
		...
		if(!empty($pdata['setting_price']))
		{
				$array_price = json_decode($pdata['setting_price'],true);
				...
				$array_price['price'] = number_format($array_price['price'],0,",",".");
				$xtpl->assign('setting_price', $array_price);
				$xtpl->parse('main.loop.price2.setting_price');
				
		
		}
		
		 // Group thuộc tính khách hàng chọn khi đặt hàng
            if (!empty($pdata['group'])) {
                $pdata['group'] = explode(',', $pdata['group']);
                foreach ($pdata['group'] as $groupid) {
                    $items = $global_array_group[$groupid];
                    $items['parent_title'] = $global_array_group[$items['parentid']]['title'];
                    $xtpl->assign('group', $items);
    ...
    $xtpl->assign('order_total', nv_number_format(($data_content['order_total'] + $price_setting), nv_get_decimals($pro_config['money_unit'])));
    $xtpl->assign('unit', $money_config[$data_content['unit_total']]['symbol']);
	
    $xtpl->assign('url_checkout', NV_BASE_SITEURL . 'index.php?' . NV_LANG_VARIABLE . '=' . NV_LANG_DATA . '&amp;' . NV_NAME_VARIABLE . '=' . $module_name . '&' . NV_OP_VARIABLE . '=checkout&order_id='.$data_content['order_id']);
    ...
    if ($pro_config['active_payment'] == '1' and $pro_config['active_order'] == '1' and $pro_config['active_price'] == '1') {
...
{
    global $module_info, $lang_module, $module_file, $pro_config, $money_config, $global_array_group , $array_location, $array_shops;;
	...
	if(!empty($data_content['coupons']))
	{
		$xtpl->assign('coupons', nv_number_format($data_content['coupons'], nv_get_decimals($pro_config['money_unit'])));
		$xtpl->assign('unit_coupons', $data_content['unit_total']);
		$xtpl->parse('main.total_coupons');
	}
    $array_group_main = array();
    if (!empty($global_array_group)) {
        foreach ($global_array_group as $array_group) {
    ...
    foreach ($data_pro as $pdata) {
		...
		if(!empty($pdata['setting_price']))
		{
				...
				if($array_price['price'] > 0)
				{
				$price_setting = $price_setting + ($array_price['price'] * $pdata['product_number']);
				$pdata['product_price'] = $pdata['product_price'] + $array_price['price'];
				$xtpl->assign('dau', '+');
				}
				else $xtpl->assign('dau', '-');
				$array_price['price'] = number_format($array_price['price'],0,",",".");
 ...
 * @return
 */
function history_order($data_content, $page)
{
 ...
 * @return
 */
function email_new_order($content, $data_content, $data_pro, $data_shipping, $data_table = false)
{
    global $module_info, $lang_module, $module_file, $pro_config, $global_config, $money_config, $global_array_group, $array_location, $array_shops;
    ...
    if ($data_table) {
		...
		if(!empty($data_content['coupons']))
		{
			$xtpl->assign('coupons', nv_number_format($data_content['coupons'], nv_get_decimals($pro_config['money_unit'])));
			$xtpl->assign('unit_coupons', $data_content['unit_total']);
			$xtpl->parse('data_product.price3.total_coupons');
		}
    ...
    if ($pro_config['use_shipping']) {
        if ($data_shipping) {
            ...
            $data_shipping['ship_shops_title'] = $array_shops[$data_shipping['ship_shops_id']]['name'];
            $xtpl->assign('DATA_SHIPPING', $data_shipping);
            $xtpl->parse('data_product.data_shipping');
        }
    } else {
        $xtpl->parse('data_product.order_address');
    }
        ...
        foreach ($data_pro as $pdata) {
			...
			if(!empty($pdata['setting_price']))
			{
					...
					$array_price['price'] = number_format($array_price['price'],0,",",".");
					$xtpl->assign('setting_price', $array_price);
					$xtpl->parse('data_product.loop.price2.setting_price');
            ...
            if (!empty($pdata['group'])) {
                ...
                foreach ($pdata['group'] as $groupid) {
                    $items = $global_array_group[$groupid];
                    $items['parent_title'] = $global_array_group[$items['parentid']]['title'];
                    $xtpl->assign('group', $items);//print_r($global_array_group);die; 
                    $xtpl->parse('data_product.loop.display_group.group');
                }
                $xtpl->parse('data_product.loop.display_group');
            }
...
function nv_template_tag($array_data, $page)
{
    global $module_info, $lang_module, $lang_global, $module_name, $module_data, $module_file, $module_upload, $pro_config, $op, $compareid,$title_tag,$description,$bodytext,$ratename,$image_tag,$ratenumber;
    ...
    if (!empty($array_data)) {
        ...
        $xtpl->assign('HEIGHT', $pro_config['homeheight']);
        $xtpl->assign('WIDTH', $pro_config['homewidth']);
		$xtpl->assign('TITLE', $title_tag);
		$xtpl->assign('BODYTEXT', $bodytext);
		
   if (! empty($description)) {
        $xtpl->assign('DESCRIPTION', $description);
        if (! empty($image_tag)) {
            $xtpl->assign('HOMEIMG1', $image_tag);
            $xtpl->parse('main.tag.image');
        }
		
		if (! empty($ratenumber)) {
       	$xtpl->assign('RATENAME', $ratename);
		$xtpl->assign('RATENUMBER', $ratenumber);
        $xtpl->parse('main.tag.rate');
        }
...
function nv_template_loadcart($array_data, $array_products = array())
{
    ...
    $xtpl->assign('TEMPLATE', $module_info['template']);
    $xtpl->assign('LINK_VIEW', nv_url_rewrite(NV_BASE_SITEURL . "index.php?" . NV_LANG_VARIABLE . "=" . NV_LANG_DATA . "&amp;" . NV_NAME_VARIABLE . "=" . $module_name . "&amp;" . NV_OP_VARIABLE . "=cart",true));
    $xtpl->assign('WISHLIST', nv_url_rewrite(NV_BASE_SITEURL . "index.php?" . NV_LANG_VARIABLE . "=" . NV_LANG_DATA . "&amp;" . NV_NAME_VARIABLE . "=" . $module_name . "&amp;" . NV_OP_VARIABLE . "=wishlist",true));
