 ...
 * @return
 */
function nv_shipping_price($total, $weight, $weight_unit, $location_id, $shops_id, $carrier_id)
{
	...
	$free_ship = $db->query('SELECT free_ship FROM ' . $db_config['prefix'] . '_' . $module_data . '_carrier_config_items WHERE id = ' . $shops_id)->fetchColumn();
		
	if($free_ship > 0 and $total > $free_ship)
	{
		$price = '0';
		return $price;
	}
