...
if(false)
{
	$list_news = $db->query('SELECT id, alias FROM ' . NV_PREFIXLANG . '_' . $module_data)->fetchAll();
	//print_r($list_news);die;
	$op_module = 'main';
	$module = $module_name;
	foreach($list_news as $a)
	{
		// ADD alias 
							$stmt = $db->prepare( 'INSERT INTO ' . NV_PREFIXLANG . '_' . $module_data . '_rows (alias, module, op, id_alias, weight, status) VALUES (:alias, :module, :op, :id_alias, :weight, :status)' );
							$stmt->bindValue( ':status', 1, PDO::PARAM_INT );
							$weight = 1;
							$stmt->bindParam( ':weight', $weight, PDO::PARAM_INT );
							$stmt->bindParam( ':alias', $a['alias'], PDO::PARAM_STR );
							$stmt->bindParam( ':module', $module, PDO::PARAM_STR );
							$stmt->bindParam( ':op', $op_module, PDO::PARAM_STR );
							$stmt->bindParam( ':id_alias', $a['id'], PDO::PARAM_INT );
							$exc = $stmt->execute();
	}
	die(ok);
}
...
if ($nv_Request->get_int('save', 'post') == '1') {
	...
	$row['ratename'] = $nv_Request->get_title('ratename', 'post', '', 1);
	$row['ratenumber'] = $nv_Request->get_title('ratenumber', 'post', '', 1);
    ...
    elseif (empty($row['title'])) {
        ...
        if ($id and !$copy) {
            $_sql = 'UPDATE ' . NV_PREFIXLANG . '_' . $module_data . ' SET title = :title, alias = :alias, image = :image, imagealt = :imagealt, imageposition = :imageposition, titlesite = :titlesite,hometext = :hometext,ratename = :ratename, ratenumber = :ratenumber,description = :description, bodytext = :bodytext, keywords = :keywords, socialbutton = :socialbutton, activecomm = :activecomm, layout_func = :layout_func, gid = :gid, edit_time = ' . NV_CURRENTTIME . ', hot_post = :hot_post WHERE id =' . $id;
            ...
            $_sql = 'INSERT INTO ' . NV_PREFIXLANG . '_' . $module_data . '
				(title, alias, image, imagealt, imageposition, titlesite,hometext,ratename,ratenumber, description, bodytext, keywords, socialbutton, activecomm, layout_func, gid, weight,admin_id, add_time, edit_time, status,hot_post) VALUES
				(:title, :alias, :image, :imagealt, :imageposition, :titlesite,:hometext,:ratename,:ratenumber, :description, :bodytext, :keywords, :socialbutton, :activecomm, :layout_func, :gid, ' . $weight . ', ' . $admin_info['admin_id'] . ', ' . NV_CURRENTTIME . ', ' . NV_CURRENTTIME . ', 1,:hot_post)';
        ...
        try {
			...
			$sth->bindParam(':ratename', $row['ratename'], PDO::PARAM_STR);
			$sth->bindParam(':ratenumber', $row['ratenumber'], PDO::PARAM_STR);
