...
if (! nv_function_exists('nv_block_doitac')) {
    /**
     * nv_block_config_doitac()
     *
     * @param mixed $module
     ...
     * @return
     */
    function nv_block_config_doitac($module, $data_block, $lang_block)
    {
    ...
    /**
     * nv_block_config_doitac_submit()
     *
     * @param mixed $module
     * @param mixed $lang_block
     * @return
     */
    function nv_block_config_doitac_submit($module, $lang_block)
    {
        global $nv_Request;
        $return = array();
        $return['error'] = array();
        $return['config'] = array();
        $return['config']['blockid'] = $nv_Request->get_int('config_blockid', 'post', 2);
     ...
     * @return
     */
    function nv_block_doitac($block_config)
    {
        ...
        if (! empty($list)) {
            if (file_exists(NV_ROOTDIR . '/themes/' . $global_config['module_theme'] . '/modules/' . $site_mods[$module]['module_file'] . '/block.doitac.tpl')) {
                $block_theme = $global_config['module_theme'];
            } elseif (file_exists(NV_ROOTDIR . '/themes/' . $global_config['site_theme'] . '/modules/' . $site_mods[$module]['module_file'] . '/block.doitac.tpl')) {
                $block_theme = $global_config['site_theme'];
            } else {
                $block_theme = 'default';
            }
            $xtpl = new XTemplate('block.doitac.tpl', NV_ROOTDIR . '/themes/' . $block_theme . '/modules/' . $site_mods[$module]['module_file']);
...
if (defined('NV_SYSTEM')) {
    $content = nv_block_doitac($block_config);
}