...
if ($nv_Request->isset_request('submit', 'post')) {
    ...
    if (! empty($data['link'])) {
        ...
        if (! nv_is_url($data['link'])) {
            $error[] = array(
                'name' => 'link',
                'value' => $lang_module['content_link_error']
            );
        }
    }
    ...
    if (empty($error)) {
        if ($data['id']) {
			...
			WHERE id = ' . $data['id'];
        } else {								$weight = $db->query("SELECT MAX(weight) FROM " . NV_PREFIXLANG . "_" . $module_data. "_rows where bid=".$bid." ")->fetchColumn();			$weight = intval($weight) + 1;                         			
            $sql = 'INSERT INTO ' . NV_PREFIXLANG . '_' . $module_data . '_rows (bid, title, description, link, target, image, start_time, end_time,weight, status) VALUES (
				' . $bid . ', :title, :description, :link, :target, :image, :start_time, :end_time, ' . $weight . ', :status
			)';
        }
