...
$sql = 'SELECT * FROM ' . NV_PREFIXLANG . '_' . $module_data . '_rows WHERE bid=' . $bid . ' ORDER BY weight ASC';
...
if ($num_rows < 1) {
   ...
   $i = 0; 
    foreach ($array as $row) {						            for ($i = 1; $i <= $num_rows; ++$i) {                $xtpl->assign('WEIGHT', array(                    'key' => $i,                    'title' => $i,                    'selected' => $i == $row['weight'] ? ' selected="selected"' : ''                ));               $xtpl->parse('main.rows.loop.weight');            }					 	
        if (! empty($row['image'])) {
